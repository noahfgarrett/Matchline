...
- **EXTO template mode.** A site hands over its own registry workbook once;
  Matchline captures that sheet's header row — any width, any wording, headers on
  whatever row they sit on — and writes every later EXTO export onto it. Columns
  the capture did not recognise are left empty in every row, so a column that is
  somebody else's to fill (contact details among them) is never written to. The
  capture holds a layout and never a data row, and is stored in the project file.
- **Learned WBS table.** One rung, keyed on the System Key alone, same 0.9 gate
  and same "below the gate is a proposal, never a guess" as the item-master
  table. Codes stay text, so a leading zero survives.
- **Model-first precedence.** `PropertyMappings` gains optional WBS, Item Master
  and Equipment Classification properties, read by the asset catalog with
  per-field provenance and flowed through the compiler. For each of those three
  the EXTO assembly prefers a non-blank mapped model value over a learned
  assignment, and a learned assignment over a blank cell. Screen 3 has pickers
  for all three; leaving one unmapped keeps today's behaviour exactly.
- **Blank, not `N/A`.** Closest Parent and Dependencies leave the cell empty when
  nothing was stated, which is what a real register carries. The donor's `'N/A'`
  rendering stays available behind an option.
- **Widened generic Rev21 map.** Eight further generic columns Matchline can fill
  honestly — Building, Level, Grid, Discipline, WBS, System Name, Manufacturer,
  Model Number — at their Rev21 positions. Every donor column kept its position.
- **Suspect-row rule narrowed.** Electrical-apparatus tokens are matched as whole
  underscore-delimited segments rather than as substrings, and `GEAR` now
  additionally requires the name's own discipline-family segment to be the
  electrical one — because `GEAR` means switchgear in an electrical name and a
  gearbox in a mechanical one. Measured against an 18k-row hand-built register
  this changes no row's verdict; it narrows what the rule can wrongly catch.
- **Project file schema v3.** Widens two CHECK constraints — a sixth `config`
  section for the captured layout, and `'wbs'` as a third learned kind. Both
  tables are rebuilt row for row, ids preserved, behind the existing opt-in
  migration with its automatic backup. A frozen v2 DDL fixture proves the
  widening is real.
