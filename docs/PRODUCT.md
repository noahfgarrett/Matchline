...
| 10 | The System Resolver settles a key and a description from the site's own sources (§5), and nothing checks either against a published vocabulary. | **The approved VF Exto vocabulary is enforced at compile and at publish.** Two resolver rungs read it: `upn-from-tag` takes the approved UPN out of the equipment tag (`MAH101-01` and `VFD101-01` are both system 101, no anatomy needed), and `exto-system-name` yields the one System Name the Upload Template accepts for that UPN, refusing with the approved candidates listed rather than printing a name Exto would reject. An opt-in `applyIcDisciplineRule` applies the SSM SOP's instrumentation rule — `I&C` is `FACILITIES MONITORING SYSTEM`, on the UPN in the instrument's own tag. Every compile publishes `completeness.approvedValues`; an unapproved UPN or Discipline blocks publication for a project whose profile says it delivers to Exto, an unapproved System Name is a warning, and the EXTO sheet prints the approved spelling of a dropdown-backed cell. The vocabulary itself is vendored from SSM-Audit and never authored here. | SSM-Audit layer 2. Exto validates its gating columns against fixed dropdowns and refuses a whole upload over one cell outside one; that refusal used to arrive after somebody exported and uploaded. |
