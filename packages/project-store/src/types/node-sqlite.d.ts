 ...
 * chain surface), so each package declares the members it calls. `node:sqlite`
 * is still experimental on Node 24 — using it emits an ExperimentalWarning at
 * runtime, which is accepted. Nothing from this file may appear in the
 * package's public API: ambient `.d.ts` inputs are not emitted to `dist/`, and
